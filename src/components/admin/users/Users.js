...
import { removeUser } from "../../../services/usuarios/usuarioService";
...
export default function Users({ usuarios, fetchDataOut }) {
      ...
      <View style={styles.buttonContainer}>
        ...
        <TouchableOpacity style={styles.iconContainer}>
          <Icon
            ...
            color={colors.primary}
            onPress={async () => {
              const response = await removeUser(usuario.uid);
              if  (response) {
                Toast.show({
                  type: "success",
    ...
    return (
      <>
        <Button
          ...
          onPress={() => {
            setRenderComponent(
              <FormUser close={openClose} fetchDataOut={fetchDataOut} />
            );
            openClose();
          }}
        />
  ...
  return (
    <View>
      ...
      />
      {usuarios.map((usuario) => renderUser(usuario))}
      <Modal isVisible={showModal} close={openClose}>
        {renderComponent}
      </Modal>
    </View>
